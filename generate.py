...
import json
import random
from utils.generate_utils import dataprocess, extract_prompts, link_prompts
from utils.dataloader import get_dataloader

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default='gossip')
parser.add_argument('--max_len', type=int, default=150)
parser.add_argument('--icl_num', type=int, default=2)
...
parser.add_argument('--triplet_model_path', default= '../huggingface/flan-t5-large')
parser.add_argument('--comet_model_path', default = '../huggingface/comet-bart-en')
parser.add_argument('--icl_path', default = './data/incontext_prompts.json')
...
for split in ['train', 'val', 'test']:
    ...
    for string in tqdm.tqdm(data):
        ...
        news_triplets = []
        t5_loss = []
        for rel in list(extract_prompts.keys()):
            # todo: improve extracting entities -> in-context prompts
            icl_text = ''
            for demon in incontext_prompt[rel]:
                icl_text += extract_prompts[rel] + demon
            # linker
            if rel in entity_level: p1, p2 = '. entity1 is ', 'and entity2 is '
            else: p1, p2 = '. event1 is ', 'and event2 is '
            # extract entity1
            extract_entity1_prompt = icl_text + extract_prompts[rel] + string[:256] + p1
            entity1, loss1 = generate_answer(extract_entity1_prompt)
            entity1 = entity1.replace(',', '')  # entity1: str
            # extract entity2
            extract_entity2_prompt = extract_entity1_prompt + entity1 + p2
            entity2, loss2 = generate_answer(extract_entity2_prompt)
            entity2 = entity2.replace(',', '')  # entity2: str
            t5_loss.append(loss1 + loss2)
            news_triplets.append([entity1, rel, entity2])  # List (rels * 3)
        # S1. commonsense reasoning
        queries = ["{} {} [GEN]".format(tri[0], tri[1]) for tri in news_triplets]
        ...
        if score > args.comet_threshold: expl = 'However, ' + news_triplets[gold_index][0] + link_prompts[news_triplets[gold_index][1]] + gold_triplet[gold_index][2] + ' instead of ' + news_triplets[gold_index][2]
        else: expl = 'Meanwhile, ' + news_triplets[gold_index][0] + link_prompts[news_triplets[gold_index][1]] + gold_triplet[gold_index][2]
