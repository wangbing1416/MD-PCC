...
class Trainer():
    ...
    def train(self, logger = None):
        ...
        train_loader = get_dataloader(self.args.data_path + 'train' + self.args.dataset_id + '.json', self.args.data_path + 'train_emo.npy',
                                      self.args.max_len, self.args.batch_size, shuffle=True, aug_prob=self.args.aug_prob, pretrain_name=self.args.pretrain_name)
        val_loader = get_dataloader(self.args.data_path + 'val' + self.args.dataset_id + '.json', self.args.data_path + 'val_emo.npy',
                                    self.args.max_len, self.args.batch_size, shuffle=False, aug_prob=self.args.aug_prob, pretrain_name=self.args.pretrain_name)
        test_loader = get_dataloader(self.args.data_path + 'test' + self.args.dataset_id + '.json', self.args.data_path + 'test_emo.npy',
                                     self.args.max_len, self.args.batch_size, shuffle=False, aug_prob=self.args.aug_prob, pretrain_name=self.args.pretrain_name)
        ...
        for epoch in range(self.args.epoch):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                pred, domain_pred = self.detector(**batch_data, alpha=alpha)
    ...
    def test(self, dataloader):
        ...
        with torch.no_grad():
            for step_n, batch in enumerate(data_iter):
                ...
                batch_pred, batch_feature = self.detector(**batch_data, alpha=-1)
